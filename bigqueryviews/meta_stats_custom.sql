-- View: level-hope-462409-a8.mkt_channels.meta_stats_custom
...
-- Updated: Added USD to EUR conversion for USD accounts using exchange rate table
...
SELECT 
  ...
  CASE 
    WHEN usd_accounts.account_id IS NOT NULL AND usdeur.rate IS NOT NULL 
    THEN spend * usdeur.rate 
    ELSE spend 
  END AS spend,
...
-- Join with USD accounts table to identify USD accounts (ensure no duplicates)
LEFT JOIN (
  SELECT DISTINCT account_id, currency, channel
  FROM `level-hope-462409-a8.utils.usd_accounts`
  WHERE account_id IS NOT NULL
) AS usd_accounts
  ON base_with_brand.account_id = usd_accounts.account_id
...
LEFT JOIN (
  ...
  ON base_with_brand.date_start = usdeur.date
