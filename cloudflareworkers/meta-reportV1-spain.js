 ...
 * SPAIN META ACCOUNT VERSION (FRM-SPAIN)
 * -------------------------------------------------------------------
 * This worker is configured for Spain Meta account (act_267879991917703)
 * with custom conversions: login_unique, deposit_unique, deposit_count, login_count
 * CRITICAL FIX: Changed from "conversions" field to "actions" field for custom conversions
 * 
 * Usage:
 * - /?account=FRM-SPAIN             → Pull for specific account (REQUIRED for manual requests)
 ...
 * - /?table=meta_stats2             → Specify BigQuery table name (default: meta_stats_es)
...
export default {
  ...
  async function handleRequest(req, env, ctx) {
    ...
    const tableName = url.searchParams.get("table") || env.BQ_TABLE_NAME || "meta_stats_es";
    ...
    // IMPORTANT: Custom conversions appear in "actions" field, NOT "conversions" field
    const FIELDS = [
      ...BASE_FIELDS,
      "actions",
      "action_values"
  ...
  // Spain account - Custom Conversions (with 14-day deduplication)
  // 
  // These custom conversions are properly returned by the Meta Insights API
  // when using the 'actions' field (confirmed via filtering test Nov 2025).
  // They include built-in 14-day deduplication logic from Meta.
  //
  // Custom conversions discovered from Meta API:
  // - login_unique: Unique logins (14-day dedup)
  // - deposit_unique: Unique deposits (14-day dedup)
  // - deposit_count: All deposit counts
  // - login_count: All login counts
  // 
  // Note: "deposit" (ID: 2594828540712125) was removed - it's ARCHIVED in Meta
  const ACTION_TYPES = [
  ...
  // Map conversion IDs to friendly names for BigQuery columns
  ...
  const shortAction = s => {
    // Extract ID from offsite_conversion.custom.XXXXXXXX format
    const match = s.match(/offsite_conversion\.custom\.(\d+)/);
    if (match && CONVERSION_NAMES[match[1]]) {
      return CONVERSION_NAMES[match[1]];
    }
    // Fallback for other formats
    return s.replace(/^offsite_conversion\.(fb_pixel_custom\.|custom\.)/, "");
  };
    ...
    const flat = (ACCOUNT_ID, r) => {
      ...
      // actions & action_values at the END (custom conversions are in actions, not conversions)
  ...
  for (const accountId of allAccountIds) {
    try {
      ...
      // Note: shortAction is already defined at the top with conversion ID mapping
      const monthBqObjects = accountRows.map(r => {
        const conversions = Array.isArray(r.actions) ? r.actions : [];
        const conversionValues = Array.isArray(r.action_values) ? r.action_values : [];
