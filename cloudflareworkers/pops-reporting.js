/**
 * Version: 1.0.0
 * POP Cloudflare Reporting – ExoClick, TrafficJunky & TrafficStars → BigQuery (BQ-only)
 * -------------------------------------------------------------------------------------
 * What it does
 *  - Authenticates to ExoClick, TrafficJunky (TJ), and/or TrafficStars (TS)
 *  - Collects stats for a date range (Exo in bulk calls, TJ day-by-day, TS per-campaign)
 ...
 *  - TS_API_KEY              : TrafficStars API key (offline token used as refresh_token)
 *  - TS_COST_MULT            : (optional) TrafficStars cost multiplier (default: 1.0)
 ...
 *  - ROLLING_WINDOW_DAYS     : (optional) Integer for cron rolling window (default: 4)
 *
 * Query Parameters
 *  - start_date  : (required) YYYY-MM-DD
 *  - end_date    : (required) YYYY-MM-DD
 *  - channel     : (optional) 'exo' | 'tj' | 'ts' | 'both' (default: 'both')
 *  - schema_migrate : (optional) 'true' | 'false' (default: 'false')
 *      When 'true', runs one-time BigQuery DDL for TJ table (ensure table
 *      exists, set options, and add missing ad-level columns). Leave 'false'
 *      for normal runs to avoid rate limits on table updates.
 *  - retry_only  : (optional) 'true' | 'false' (default: 'false')
 *      When 'true', skips discovery and processes only previously failed
 *      campaign/day tasks loaded from KV (see KV binding below).
 *  - backfill    : (optional) 'true' | 'false' (default: 'false')
 *      When 'true', split the requested date range into chunks (see chunk_days)
 *      and enqueue them into KV for scheduled/background processing.
 *  - chunk_days  : (optional) integer (default: 2)
 *      Number of days per chunk when backfill=true. Example: 2 → split into 2-day windows.
 *  - drain_next  : (optional) 'true' | 'false' (default: 'false')
 *      When 'true', process the next enqueued backfill chunk immediately (if any).
 *
 * KV Namespaces (Bindings)
 *  - TJ_RETRY_KV           : KV used to store/recall failed TJ tasks between runs
 *                             Key format: `tj:retry:${start_date}:${end_date}`
 *                             Value: JSON array of { date, campaignId, campaignName, reason }
 *  - Also used for backfill queue under key `tj:backfill:queue` storing an array
 *    of tasks: { start_date, end_date, channel }
 *
 * Data Collection Logic
 *  - ExoClick: Single bulk API call grouped by date,campaign_id,site_id,variation_id
 *    Falls back to per-campaign calls if bulk grouping fails
 *  - TrafficJunky: Day-by-day processing with campaign discovery
 *    1. Discover campaigns via /api/campaigns/bids/stats.json (range or per-day)
 *    2. For each campaign/day: try /api/ads/{campaignId}/stats/one.json (ad-level)
 *    3. If no ad stats: fallback to /api/bids/{campaignId}.json (bid-level)
 *    4. Bid fallback creates rows with null ad_id, populated spot_id/spot_name
 *  - TrafficStars: Per-campaign processing with by-spot and by-creative endpoints
 *    1. Get campaigns list via /v1.1/campaigns
 *    2. For each campaign: fetch by-spot stats and by-creative metadata
 *    3. Merge creative URLs into spot-level rows
 *
 * API Rate Limiting & Retry Logic
 *  - TJ API calls include global rate limit (1200ms gap) + exponential backoff on 429/5xx
 *  - TJ campaign "discovery" is done once per date range to reduce API calls; we
 *    fallback to per-day discovery only if that fails
 *  - On partial failures, successful rows are still staged/merged; failures are
 *    persisted to KV for later retry (or run with retry_only=true)
 *
 * BigQuery Tables
 *  - exo_stats(date DATE, campaign_id STRING, campaign_name STRING,
 ...
 *
 *  - tj_stats(date DATE, campaign_id STRING, campaign_name STRING,
 *             ad_id STRING, ad_name STRING, target_url STRING,
 *             spot_id STRING, spot_name STRING,
 *             impressions INT64, clicks INT64, conversions INT64,
 *             revenue FLOAT64, cost FLOAT64, retrieved_at TIMESTAMP)
 *    KEY: (date, campaign_id, ad_id, spot_id)  Partition: date  Cluster: campaign_id,ad_id,spot_id
 *    Note: ad_id/spot_id can be null for bid-level fallback data
 *
 *  - ts_stats_siteid(date DATE, campaign_id STRING, campaign_name STRING,
 *             site_id STRING, site_name STRING, creative_url STRING, banner_id STRING,
 *             impressions INT64, clicks INT64, conversions INT64,
 *             cost FLOAT64, retrieved_at TIMESTAMP)
 *    KEY: (date, site_id)  Partition: date  Cluster: site_id
 */
...
function last4WindowUTC() {
  ...
  const from = new Date(todayUTC.getTime() - 3*24*3600*1000);
  return { 
    FROM: formatYmd(from), 
    TO: formatYmd(to) 
  };
...
export default {
  async fetch(request, env, ctx) {
    ...
    // TrafficStars Env (offline token aka API key)
    const TS_API_KEY            = env.TS_API_KEY; // offline token used as refresh_token per docs
    const TS_COST_MULT          = Number(env.TS_COST_MULT || 1.0);
    ...
    const channel    = (url.searchParams.get("channel") || "both").toLowerCase(); // exo | ts | both
    ...
    const wantTs  = channel === "both" || channel === "ts" || channel === "trafficstars";

    try {
      // No TJ backfill handling in this worker
      ...
      // Tables & schemas
      const exoTableId = "exo_stats";
      const tsTableId  = "ts_stats_siteid";
      ...
      // TJ schema removed (handled in separate TJ worker)
      ...
      // No TJ table migration in this worker
      if (wantTs) {
        console.log(`[BQ_TABLE] Ensuring TrafficStars table exists: ${tsTableId}`);
        await ensureBQTable(bqToken, env, tsTableId, TS_SCHEMA, "date", ["site_id"]);
        await bqSetNoExpiry(bqToken, env, tsTableId, logs);
        await bqQuery(bqToken, env,
          `ALTER TABLE \`${BQ_PROJECT_ID}.${BQ_DATASET}.${tsTableId}\`
             ADD COLUMN IF NOT EXISTS creative_url STRING`, logs).catch(() => {});
        await bqQuery(bqToken, env,
          `ALTER TABLE \`${BQ_PROJECT_ID}.${BQ_DATASET}.${tsTableId}\`
             ADD COLUMN IF NOT EXISTS banner_id STRING`, logs).catch(() => {});
        // Attempt to drop deprecated campaign_url column if present
        await bqQuery(bqToken, env,
          `ALTER TABLE \`${BQ_PROJECT_ID}.${BQ_DATASET}.${tsTableId}\` DROP COLUMN IF EXISTS campaign_url`, logs).catch(() => {});
        console.log(`[BQ_TABLE] TrafficStars table ready`);
      }

      // No TJ backfill enqueue in this worker
      ...
      // No TJ collection in this worker

      if (wantTs) {
        console.log(`[TRAFFICSTARS] Starting per-campaign data collection for ${start_date} to ${end_date}`);
        logs.push(`[TrafficStars] Fetching campaigns and by-spot/by-creative data`);
        // Acquire token
        const tsToken = await tsGetAccessToken({ apiKey: TS_API_KEY });
        console.log(`[TRAFFICSTARS] Token acquired`);
        // Fetch campaigns for id->name
        const tsCampaigns = await tsFetchCampaigns(tsToken);
        const campaignList = Array.isArray(tsCampaigns) ? tsCampaigns : [];
        console.log(`[TRAFFICSTARS] Loaded ${campaignList.length} campaigns`);
        if (campaignList.length) {
          console.log(`[TRAFFICSTARS] Campaigns sample: ${JSON.stringify(campaignList.slice(0,3))}`);
        }
        const campaignIdToName = {};
        for (const c of campaignList) {
          if (c && c.id != null) campaignIdToName[String(c.id)] = c.name || String(c.id);
        }
        // Iterate campaigns and fetch spot stats and creatives
        for (const c of campaignList) {
          const cid = c?.id;
          if (cid == null) continue;
          const cidStr = String(cid);
          // by-spot
          const bySpot = await tsFetchBySpot(tsToken, start_date, end_date, cid);
          if (bySpot && typeof bySpot === 'object') {
            const sampleKeys = Object.keys(bySpot).slice(0,3);
            const sample = sampleKeys.map(k => bySpot[k]);
            if (sample.length) console.log(`[TRAFFICSTARS] by-spot sample for campaign ${cidStr}: ${JSON.stringify(sample)}`);
          }
          for (const k of Object.keys(bySpot || {})) {
            const r = bySpot[k] || {};
            const ymd = r.day || r.date || null;
            const siteId = r.spot_id != null ? String(r.spot_id) : null;
            const siteName = r.spot_name || r.spot || null;
            const impressions = toNumber(r.impressions);
            const clicks = toNumber(r.clicks);
            const conversions = toNumber(r.leads || r.conversions);
            const rawAmount = Number(r.amount || r.cost || 0);
            const cost = Number((rawAmount * TS_COST_MULT).toFixed(6));
            tsObjects.push({
              date: ymd || start_date,
              campaign_id: cidStr,
              campaign_name: campaignIdToName[cidStr] || `Campaign ${cidStr}`,
              site_id: siteId,
              site_name: siteName,
              creative_url: null,
              banner_id: null,
          ...
          // by-creative for URLs
          const byCreative = await tsFetchByCreative(tsToken, start_date, end_date, cid);
          if (Array.isArray(byCreative) && byCreative.length) {
            console.log(`[TRAFFICSTARS] by-creative sample for campaign ${cidStr}: ${JSON.stringify(byCreative.slice(0,3))}`);
          }
          const { url: creativeUrl, bannerId } = extractCreativeMeta(byCreative);
          console.log(`[TRAFFICSTARS] URL enrichment for campaign ${cidStr}: creative_url=${creativeUrl || 'null'}, banner_id=${bannerId || 'null'}`);
          // apply URLs to rows for this campaign (scan all to be safe)
          for (let i = 0; i < tsObjects.length; i++) {
            const row = tsObjects[i];
            if (row.campaign_id === cidStr) {
              if (!row.creative_url && creativeUrl) row.creative_url = creativeUrl;
              if (!row.banner_id && bannerId) row.banner_id = bannerId;
            }
          }
        }
        console.log(`[TRAFFICSTARS] Processed ${tsObjects.length} rows for BigQuery`);
        logs.push(`[TrafficStars] Collected ${tsObjects.length} rows with retrievedAt=${retrievedAt}`);
      }

      // === Stage tables (TTL) & load NDJSON ===
      const exoStageId = wantExo ? `exo_stats_stage_${stageSuffix}` : null;
      const tsStageId  = wantTs  ? `ts_stats_stage_${stageSuffix}`  : null;
      console.log(`[BQ_STAGE] Stage suffix: ${stageSuffix}, Exo stage: ${exoStageId}`);
      ...
      // No TJ staging in this worker
      if (wantTs) {
        console.log(`[BQ_STAGE] Creating TrafficStars stage table: ${tsStageId}`);
        await bqCreateStageTable(bqToken, env, tsStageId, TS_SCHEMA, "date", ["site_id"], BQ_STAGE_TTL_MIN);
        console.log(`[BQ_STAGE] Loading ${tsObjects.length} TrafficStars rows to stage table...`);
        const outRows = await bqLoadJson(bqToken, env, tsStageId, tsObjects, TS_SCHEMA, logs);
        console.log(`[BQ_STAGE] TrafficStars: staged ${outRows} row(s) successfully`);
        logs.push(`TrafficStars: staged ${outRows} row(s)`);
      }
      ...
      // No TJ merge/verify in this worker
      if (wantTs && tsObjects.length) {
        console.log(`[BQ_MERGE] Starting TrafficStars merge: ${tsObjects.length} rows -> ${tsTableId}`);
        logs.push(`[MERGE] Starting TrafficStars merge: ${tsObjects.length} rows -> ${tsTableId}`);
        const sql = mergeSQL({
          project: BQ_PROJECT_ID, dataset: BQ_DATASET,
          table: tsTableId, stage: tsStageId,
          keyFields: ["date","site_id"],
          nonKeyFields: ["campaign_id","campaign_name","site_name","creative_url","banner_id","impressions","clicks","conversions","cost","retrieved_at"],
        });
        console.log(`[BQ_MERGE] TrafficStars SQL: ${sql.substring(0, 200)}...`);
        await bqQuery(bqToken, env, sql, logs);
        console.log(`[BQ_MERGE] TrafficStars merge completed successfully`);
        logs.push("TrafficStars: merge completed");

        console.log(`[BQ_VERIFY] Checking TrafficStars row count...`);
        const verify = targetCountSQL(BQ_PROJECT_ID, BQ_DATASET, tsTableId, start_date, end_date);
        const cntRows = await bqQuery(bqToken, env, verify.sql, logs, /*returnRows*/true);
        console.log(`[BQ_VERIFY] TrafficStars rows in range [${start_date}..${end_date}] = ${cntRows?.[0]?.rows_in_range ?? 'n/a'}`);
        logs.push(`TrafficStars: rows in range [${start_date}..${end_date}] = ${cntRows?.[0]?.rows_in_range ?? 'n/a'}`);
      }

      // === Best-effort cleanup ===
      console.log(`[BQ_CLEANUP] Starting cleanup of stage tables...`);
      if (wantExo) await bqDropTable(bqToken, env, exoStageId).catch(() => {});
      if (wantTs)  await bqDropTable(bqToken, env, tsStageId).catch(() => {});
      ...
      if (wantTs)  sum.push(`TrafficStars: staged ${tsObjects.length} row(s), merged OK.`);
  ...
  async scheduled(event, env, ctx) {
    ...
    try {
      // Run rolling window for configured days (default 4) for TS (or both)
      ...
      // Default to ts (or both) now that TJ moved to separate worker
      base.searchParams.set("channel", "both");

      // No KV gating here; TJ backfill logic moved to TJ worker
...
// (All TrafficJunky and TJ KV helper logic removed; handled by TJ worker)

/** ===================== TRAFFICSTARS ===================== **/
async function tsGetAccessToken({ apiKey }) {
  // Prefer offline token (API key) via refresh_token grant per docs
  if (apiKey) {
    const body = new URLSearchParams({ grant_type: 'refresh_token', refresh_token: apiKey });
    const res = await fetch('https://api.trafficstars.com/v1/auth/token', {
      method: 'POST',
    ...
    throw new Error(`TrafficStars refresh_token flow failed: ${res.status} ${JSON.stringify(json)}`);
  }
  throw new Error('TrafficStars authentication not configured: provide TS_API_KEY');
}
async function tsFetchBySpot(token, dateFrom, dateTo, campaignId) {
  let url = `https://api.trafficstars.com/v1.1/advertiser/custom/report/by-spot?date_from=${encodeURIComponent(dateFrom)}&date_to=${encodeURIComponent(dateTo)}`;
  if (campaignId != null) url += `&campaign_id=${encodeURIComponent(campaignId)}`;
  const res = await fetch(url, {
  ...
  if (!res.ok) throw new Error(`TrafficStars by-spot failed ${res.status}: ${text}`);
  return json || {};
}

async function tsFetchCampaigns(token) {
  const url = `https://api.trafficstars.com/v1.1/campaigns`;
  const res = await fetch(url, { headers: { 'Authorization': `bearer ${token}` } });
  const text = await res.text();
  let json = null;
  try { json = JSON.parse(text); } catch { json = null; }
  if (!res.ok) throw new Error(`TrafficStars campaigns failed ${res.status}: ${text}`);
  const arr = json?.response || json || [];
  return Array.isArray(arr) ? arr : [];
}

async function tsFetchByCreative(token, dateFrom, dateTo, campaignId) {
  let url = `https://api.trafficstars.com/v1.1/advertiser/custom/report/by-creative?date_from=${encodeURIComponent(dateFrom)}&date_to=${encodeURIComponent(dateTo)}`;
  if (campaignId != null) url += `&campaign_id=${encodeURIComponent(campaignId)}`;
  const res = await fetch(url, { headers: { 'Authorization': `bearer ${token}` }, method: 'GET' });
  const text = await res.text();
  let json = null;
  try { json = JSON.parse(text); } catch { json = null; }
  if (!res.ok) throw new Error(`TrafficStars by-creative failed ${res.status}: ${text}`);
  return json || [];
}

function extractCreativeMeta(creativeRows) {
  if (!Array.isArray(creativeRows)) return { url: null, bannerId: null };
  for (const r of creativeRows) {
    const url = r?.url || r?.click_url || r?.landing_url || r?.resource_url || null;
    const bannerId = r?.banner_id != null ? String(r.banner_id) : null;
    if (url || bannerId) return { url: url ? String(url) : null, bannerId };
  }
  return { url: null, bannerId: null };
}
